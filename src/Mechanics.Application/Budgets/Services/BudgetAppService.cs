    ...
    , IBudgetAppService
{
    ...
    ///     Aprova um orçamento.
    /// </summary>
    public async Task ApproveBudget(Guid workOrderId, string? description = null,
    CancellationToken cancellationToken = default)
    {
        ...
        {
            WorkOrderId = wo.Id,
            Action = "BudgetApprovedAsync",
            Details = description ?? "Budget approved via async event",
        ...
        if (wo.Customer != null)
        {
            try
            {
                await emailService.SendWorkOrderStatusChanged(wo.Customer, wo, previousStatus, cancellationToken);
                AppMetrics.EmailsSent.Add(1, new TagList { { "template", "budget_approved_async" } });
            }
            catch (Exception ex)
            {
                AppMetrics.EmailsFailed.Add(1, new TagList { { "template", "budget_approved_async" } });
                logger.LogWarning(ex, "Failed to send email after budget approval for WorkOrder {WorkOrderId}", wo.Id);
            }
        }
    ...
    /// </summary>
    public async Task RejectBudget(Guid workOrderId, string? description = null,
    CancellationToken cancellationToken = default)
    {
        var wo = await dbContext.WorkOrders
            .Include(w => w.Customer)
        ...
        var tags = new TagList
    {
        ...
        {
            WorkOrderId = wo.Id,
            Action = "BudgetRejectedAsync",
            Details = description ?? "Budget rejected via async event",
            PerformedByUserId = null,
        };
        await dbContext.WorkOrderHistories.AddAsync(hist, cancellationToken);
        ...
        if (wo.Customer != null)
        {
            ...
            {
                await emailService.SendWorkOrderStatusChanged(wo.Customer, wo, previousStatus, cancellationToken);
                AppMetrics.EmailsSent.Add(1, new TagList { { "template", "budget_rejected_async" } });
            }
            catch (Exception ex)
            {
                AppMetrics.EmailsFailed.Add(1, new TagList { { "template", "budget_rejected_async" } });
                logger.LogWarning(ex, "Failed to send status changed email after async budget rejection for WorkOrder {WorkOrderId}", wo.Id);
            }
        }
        ...
        if (wo.AssignedToUserId != null)
        {
            ...
            try
            {
                ...
                AppMetrics.EmailsSent.Add(1, new TagList { { "template", "budget_rejected_async_mechanic" } });
            }
            catch (Exception ex)
            {
                AppMetrics.EmailsFailed.Add(1, new TagList { { "template", "budget_rejected_async_mechanic" } });
